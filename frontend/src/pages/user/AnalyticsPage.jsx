...
const PRIMARY_TABS = [
  ...
  { id: "questions",   label: "Chi ti?t câu h?i",   icon: Target },
  { id: "responses",   label: "Danh sách ph?n h?i", icon: Users },
];

const MORE_TABS = [
  { id: "gender",     label: "Gi?i tính",           icon: Venus },
...
function chiSquareTest(rows) {
  ...
  let strength = "Không"; if (v > 0.1) strength = "Y?u"; if (v > 0.3) strength = "Trung bình"; if (v > 0.5) strength = "M?nh"; if (v > 0.7) strength = "R?t m?nh";
  return {
    ...
    significance: chiSq > 10.83 ? "C?c k? có ý nghia" : chiSq > 6.63 ? "R?t có ý nghia" : chiSq > 3.84 ? "Có ý nghia" : "Không có ý nghia",
...
function NPSCard({ npsData }) {
  if (!npsData) return (
    <div style={{ background: "#FFFFFF", border: "1px solid #E8E6F0", borderRadius: 12, padding: 20, textAlign: "center"}}>
      <p style={{ color: "#9CA3AF", fontSize: 13, fontWeight: 400 }}>C?n câu h?i dánh giá (RATING 1-10) d? tính NPS</p>
    </div>
  );
        ...
        <div style={{ padding: "0 20px 20px", borderTop: "1px solid #E8E6F0" }}>
            ...
            <div style={{ paddingTop: 18 }}>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 10, marginBottom: 14 }}>
                {[
                  ...
                  { label: "Ð? l?ch", value: question.stddev?.toFixed(2), color: "#374151" },
                ].map((item, i) => (
          ...
          {isDate && (
            <div style={{ paddingTop: 18 }}>
              ...
              ) : <p style={{ color: "#9CA3AF", textAlign: "center", padding: "20px 0", fontWeight: 400 }}>Chua có d? li?u ngày</p>}
            </div>
          )}
                ...
                <>
                  {question.word_frequency?.length > 0 && (
                    <div style={{ marginBottom: 14 }}>
                      <p style={{ fontSize: 12, color: "#9CA3AF", marginBottom: 10, fontWeight: 500 }}>T? khóa n?i b?t</p>
                  ...
                  {question.pagination && (
                    <p style={{ fontSize: 12, color: "#9CA3AF", textAlign: "center", marginTop: 12, fontWeight: 400 }}>
                      Hi?n th? {question.answers.length} / {question.pagination.total_answers} câu tr? l?i
                    </p>
                  )}
                </>
              ) : <div style={{ textAlign: "center", padding: "20px 0", color: "#9CA3AF", fontWeight: 400 }}>Chua có câu tr? l?i</div>}
            </div>
          )}
        </div>
      )}
...
function TrendSwitcher({ value, onChange }) {
  const opts = [{ label: "Ngày", value: "day" }, { label: "Tu?n", value: "week" }, { label: "Tháng", value: "month" }];
  return (
...
// --- SEARCH BAR ----------------------------------------------------------------
function SearchBar({ value, onChange, placeholder = "Tìm ki?m ph?n h?i..." }) {
...
function StatusFilter({ value, onChange }) {
  const opts = [{ label: "T?t c?", value: "" }, { label: "Hoàn thành", value: "COMPLETED" }, { label: "Ðang làm", value: "IN_PROGRESS" }];
  return (
...
export default function UserAnalyticsPage() {
  ...
  const fetchCrossTab = useCallback(async () => {
    ...
    } catch { toast.error("Không t?i du?c cross-tab"); }
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <h1 style={{ fontSize: 22, fontWeight: 600, color: "#111827", margin: 0 }}>Phân tích Kh?o sát</h1>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 16 }}>
                {sLoad ? [1,2,3,4,5].map(i => <SkeletonStatCard key={i} theme="light" />) : (
                  <>
                    ...
                    <StatCard label="T?ng câu h?i" value={questions.length} icon={Target} color="cyan" delay={320} />
                  </>
                )}
              </div>
            </RetrySection>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "3fr 2fr", gap: 24 }}>
              <RetrySection error={tErr} onRetry={fetchTrend} isLoading={tLoad}>
                <div style={{ ...cardStyle, padding: "24px" }}>
                  ...
                  {tLoad ? <Shimmer height={240} /> : trend?.trend?.length > 0 ? (
                    ...
                    <EmptyState
                      icon={BarChart3}
                      title="Chua có d? li?u"
                      desc="Kh?o sát chua có ph?n h?i nào"
                      action={{ label: "Làm m?i", onClick: fetchTrend }}
                    />
                  )}
                </div>
              </RetrySection>
              ...
              <RetrySection error={hmErr} onRetry={fetchHeatmap} isLoading={hmLoad}>
                <div style={{ ...cardStyle, padding: "24px" }}>
                  ...
                  ) : <EmptyState icon={Calendar} title="Không có d? li?u" desc="Chua có ph?n h?i nào" />}
                </div>
              </RetrySection>
            </div>
              ...
              <RetrySection error={cErr} onRetry={fetchComp} isLoading={cLoad}>
                <div style={{ ...cardStyle, padding: "24px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                    ...
                    <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>Drop-off theo câu h?i</h3>
                  </div>
                  {cLoad ? <Shimmer height={200} /> : comp?.drop_off_by_question?.length > 0 ? (
                    <FunnelViz dropOffData={comp.drop_off_by_question} />
                  ) : <EmptyState icon={TrendingDown} title="Chua có d? li?u drop-off" desc="Không có ph?n h?i chua hoàn thành" />}
                </div>
              </RetrySection>
            </div>
              ...
              <RetrySection error={svErr} onRetry={fetchSurvey} isLoading={svLoad}>
                <div style={{ ...cardStyle, padding: "24px" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                    ...
                    <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>Phân b? câu h?i</h3>
                  </div>
                  ...
                  ) : <EmptyState icon={BarChart3} title="Chua có câu h?i d?ng l?a ch?n" desc="Survey không có câu h?i l?a ch?n" />}
                </div>
              </RetrySection>
            </div>
          </div>
        ...
        {activeTab === "questions" && (
          <RetrySection error={svErr} onRetry={fetchSurvey} isLoading={svLoad}>
            {svLoad ? (
              ...
              <EmptyState icon={Target} title="Chua có câu h?i" desc="Kh?o sát này chua có câu h?i nào" />
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 6 }}>
                  <p style={{ fontSize: 13, color: "#9CA3AF", margin: 0, fontWeight: 400 }}>{questions.length} câu h?i</p>
                  <div style={{ display: "flex", gap: 6 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ flex: 1, minWidth: 200 }}>
                <SearchBar value={rSearch} onChange={v => { setRSearch(v); setRPage(1); }} placeholder="Tìm ki?m trong câu tr? l?i..." />
              </div>
              ...
              <div style={{ ...cardStyle, overflow: "hidden" }}>
                <div style={{ padding: "16px 20px", borderBottom: "1px solid #E8E6F0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                    ...
                    <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>Danh sách ph?n h?i</h3>
                  </div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, padding: "10px 18px", borderBottom: "1px solid #E8E6F0", fontSize: 10, fontWeight: 500, color: "#9CA3AF", textTransform: "uppercase", letterSpacing: "0.08em" }}>
                  ...
                  <div style={{ flex: 1 }}>Tr?ng thái</div>
                ...
                {rLoad ? [1,2,3,4,5].map(i => <SkeletonTableRow key={i} cols={5} theme="light" />)
                  ...
                  : <div style={{ padding: "60px 20px" }}><EmptyState icon={rSearch || rStatus ? EyeOff : Eye} title={rSearch || rStatus ? "Không tìm th?y" : "Chua có ph?n h?i"} desc={rSearch || rStatus ? "Th? thay d?i b? l?c" : "Chua có ph?n h?i nào"} /></div>
                }
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{ ...cardStyle, padding: "24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                ...
                <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>Phân tích theo Gi?i tính & Ð? tu?i</h3>
              </div>

              <div>
                <label style={{ display: "block", fontSize: 12, color: "#9CA3AF", fontWeight: 500, marginBottom: 8 }}>
                  Ch?n câu h?i l?a ch?n d? phân tích:
                </label>
                <select
                ...
                >
                  <option value="">— Ch?n câu h?i —</option>
                  {choiceQs.map(q => (
              ...
              {!genderTabQuestion && (
                <div style={{ marginTop: 20, padding: "32px 20px", textAlign: "center", color: "#9CA3AF", fontSize: 13, fontWeight: 400 }}>
                  Vui lòng ch?n câu h?i l?a ch?n d? xem phân tích theo gi?i tính và d? tu?i.
                </div>
              )}
            </div>
                ...
                <RetrySection error={gErr} onRetry={fetchGenderData} isLoading={gLoad}>
                  <div style={{ ...cardStyle, padding: "24px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                      <Venus size={17} color="#ec4899" />
                      <div>
                        <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>So sánh theo Gi?i tính</h3>
                        <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontWeight: 400 }}>Phân b? câu tr? l?i theo Nam / N? / Khác</p>
                      </div>
                    </div>
                    ...
                    {gLoad ? <Shimmer height={260} /> : genderData && Object.keys(genderData).length > 0 ? (
                      <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                        {Object.entries(genderData).map(([gender, genderInfo], gi) => {
                          ...
                          const genderLabels = { MALE: "Nam", FEMALE: "N?", OTHER: "Khác", UNKNOWN: "Chua c?p nh?t" };
                      ...
                      <EmptyState icon={Venus} title="Chua có d? li?u" desc="Chua có ph?n h?i nào cho câu h?i này" />
                    )}
                  </div>
                </RetrySection>
                ...
                <RetrySection error={aErr} onRetry={fetchAgeData} isLoading={aLoad}>
                  <div style={{ ...cardStyle, padding: "24px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                      ...
                      <div>
                        <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>So sánh theo Ð? tu?i</h3>
                        <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontWeight: 400 }}>Phân b? câu tr? l?i theo nhóm tu?i</p>
                      </div>
                    </div>
                    ...
                    {aLoad ? <Shimmer height={260} /> : ageData && Object.keys(ageData).length > 0 ? (
                      ...
                      <EmptyState icon={Clock} title="Chua có d? li?u" desc="Chua có ph?n h?i nào" />
                    )}
                  </div>
                </RetrySection>
                ...
                <RetrySection error={iErr} onRetry={fetchInsightData} isLoading={iLoad}>
                  <div style={{ ...cardStyle, padding: "24px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                      ...
                      <div>
                        <h3 style={{ fontSize: 15, fontWeight: 600, color: "#111827", margin: 0 }}>Heatmap: Tu?i × Gi?i tính</h3>
                        <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontWeight: 400 }}>Tuong quan chi ti?t gi?a nhóm tu?i và gi?i tính</p>
                      </div>
                    </div>
                    ...
                    {iLoad ? <Shimmer height={300} /> : insightData?.insight && Object.keys(insightData.insight).length > 0 ? (
                      <div style={{ overflowX: "auto" }}>
                        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                          <thead>
                            <tr style={{ background: "#F4F3F8" }}>
                              ...
                              {["Nam", "N?", "Khác", "Chua c?p nh?t"].map(g => (
                        ...
                        <p style={{ fontSize: 11, color: "#9CA3AF", marginTop: 12, textAlign: "right", fontWeight: 400 }}>
                          * % = t? l? ph?n h?i trong nhóm tu?i × gi?i tính tuong ?ng
                        </p>
                      </div>
                    ) : (
                      <EmptyState icon={Activity} title="Chua có d? li?u" desc="Chua có ph?n h?i nào" />
                    )}
                  </div>
                </RetrySection>
              </>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            <div style={{ ...cardStyle, padding: "24px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 16, marginBottom: 16 }}>
                {[{ label: "Câu h?i A (Hàng)", val: selectedQ1, set: setSelectedQ1 },
                  { label: "Câu h?i B (C?t)", val: selectedQ2, set: setSelectedQ2 }].map(({ label, val, set }, idx) => (
                  <div key={idx}>
              ...
              <div style={{ ...cardStyle, padding: "24px", border: chiResult.has_correlation ? "1px solid #10b981" : "1px solid #E8E6F0" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontWeight: 400 }}>M?i tuong quan gi?a 2 câu h?i</p>
                  </div>
                </div>
                ...
                <div style={{ display: "flex", gap: 12, flexWrap: "wrap" }}>
                  {[
                    { label: "Ý nghia th?ng kê", value: chiResult.significance, color: chiResult.has_correlation ? "#10b981" : "#f59e0b" },
                    { label: "Ð? m?nh tuong quan", value: chiResult.strength, color: chiResult.has_correlation ? "#10b981" : "#9CA3AF" },
                    { label: "K?t lu?n", value: chiResult.has_correlation ? "Có tuong quan ?" : "Không có tuong quan", color: chiResult.has_correlation ? "#10b981" : "#f59e0b" },
                  ].map(item => (
            ...
            <div style={{ ...cardStyle, padding: "24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 11, color: "#9CA3AF", margin: 0, fontWeight: 400 }}>Phân tích thông minh du?c t?o b?i AI</p>
                </div>
                <button
              ...
              {aiLoad ? (
                ...
                <EmptyState
                  ...
                  desc="AI s? phân tích d? li?u kh?o sát và dua ra các insights."
                />
              )}
            </div>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ ...cardStyle, padding: "24px" }}>
              ...
              <p style={{ fontSize: 13, color: "#9CA3AF", marginBottom: 20, lineHeight: 1.6, fontWeight: 400 }}>
                Xu?t toàn b? d? li?u kh?o sát theo kho?ng th?i gian dã ch?n.
              </p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                {[
                  { icon: FileSpreadsheet, title: "Xu?t CSV", desc: "Ð?nh d?ng Excel. Phù h?p phân tích trong Excel, Google Sheets.", color: "#10b981" },
                  { icon: Copy, title: "Xu?t JSON", desc: "D? li?u thô JSON. Luu tr?, backup ho?c chuy?n d?i.", color: "#a855f7" },
                ].map(item => {
                  ...
                  return (
                    <div key={item.title} style={{
                      ...
                      onClick={item.title === "Xu?t CSV" ? () => toast.info("CSV export s? d?ng backend endpoint. API dã s?n sàng.") : handleExportJSON}
                    >
            ...
            <div style={{ ...cardStyle, padding: "24px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12 }}>
                {[
                  ...
                  { label: "Câu h?i", value: questions.length },
                  { label: "T? l? hoàn thành", value: `${stats?.overview?.completion_rate || 0}%` },
                ].map(item => (
                  <div key={item.label} style={{ background: "#F4F3F8", border: "1px solid #E8E6F0", borderRadius: 12, padding: "14px", textAlign: "center" }}>
                    <p style={{ fontSize: 10, color: "#9CA3AF", marginBottom: 6, fontWeight: 500 }}>{item.label}</p>

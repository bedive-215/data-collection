...
const Q_TYPES = [
  ...
  { value:"PARAGRAPH",       label:"Ðo?n van",           icon:<AlignLeft size={15}/> },
  ...
  { value:"LINEAR_SCALE",    label:"Ph?m vi tuy?n tính", icon:<ToggleLeft size={15}/> },
  ...
  { value:"FILE_UPLOAD",     label:"T?i t?p lên",        icon:<FileUp size={15}/> },
];
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <button onClick={() => setAdding(v => !v)} title="Thêm trang m?i"
          style={{
            display: "flex", alignItems: "center", justifyContent: "center",
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {toolbarBtn("underline",<Underline size={13}/>,"G?ch chân")}
        ...
        {toolbarBtn("insertOrderedList",   <span style={{fontSize:12,fontWeight:700}}>1.</span>,  "Danh sách s?")}
        ...
        >
          <option value="p">Ðo?n van</option>
          <option value="h1">Tiêu d? 1</option>
          <option value="h2">Tiêu d? 2</option>
          <option value="h3">Tiêu d? 3</option>
        </select>
...
function QuestionImageUploadArea({ image, onImageChange }) {
  ...
  return (
    <>
      ...
      >
        <ImagePlus size={16}/>
        <span>Thêm ?nh cho câu h?i · <span style={{color:C.textDim,fontSize:11}}>UI only</span></span>
      </div>
    </>
  );
}
    ...
    <div style={{
      ...
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{display:"flex",gap:4}}>
          ...
          <button
            type="button"
            title="Thêm ?nh l?a ch?n (UI only)"
            onClick={()=>fileRef.current?.click()}
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:8,paddingTop:10,paddingLeft:28}}>
        ...
        >
          <PlusCircle size={14}/> Thêm l?a ch?n
        </button>
      </div>
...
function SettingsEditor({ type, settings, onChange }) {
  if (type === "TEXT" || type === "PARAGRAPH") {
    return (
      <div>
        <span style={lbl}>Gi?i h?n ký t?</span>
  ...
  if (type === "LINEAR_SCALE") {
    ...
    return (
      <div>
        <span style={lbl}>Ph?m vi tuy?n tính</span>
        <div style={{display:"flex",gap:12}}>
          <div style={{flex:1}}>
            <span style={{fontSize:11,color:C.textSub,display:"block",marginBottom:4}}>Min</span>
            <input
        ...
        <div style={{display:"flex",gap:12,marginTop:8}}>
          <div style={{flex:1}}>
            ...
            <input type="text" value={settings?.min_label ?? ""} placeholder="Ví d?: Không hài lòng"
          ...
          <div style={{flex:1}}>
            ...
            <input type="text" value={settings?.max_label ?? ""} placeholder="Ví d?: R?t hài lòng"
  ...
  if (type === "RATING") {
    return (
      <div>
        <span style={lbl}>Ph?m vi dánh giá</span>
        <div style={{display:"flex",gap:12}}>
          <div style={{flex:1}}>
            <span style={{fontSize:11,color:C.textSub,display:"block",marginBottom:4}}>Min</span>
            <input
  ...
  if (type === "FILE_UPLOAD") return (
    <div style={{padding:"14px 0",color:C.textDim,fontSize:13}}>
      <div style={{border:"1.5px dashed " + C.border,borderRadius:10,padding:"16px 20px",textAlign:"center"}}>
        ...
        <div>Ngu?i dùng có th? t?i t?p lên t?i dây</div>
      </div>
    </div>
  );
...
const OPERATORS = [
  ...
  { value: "not_equals",    label: "không b?ng",  types: ["TEXT","PARAGRAPH","EMAIL","NUMBER","SINGLE_CHOICE","DROPDOWN","LINEAR_SCALE"] },
  { value: "contains",      label: "ch?a",         types: ["TEXT","PARAGRAPH","EMAIL"] },
  { value: "not_contains",  label: "không ch?a",  types: ["TEXT","PARAGRAPH","EMAIL"] },
  { value: "greater",       label: "l?n hon",      types: ["NUMBER","LINEAR_SCALE","RATING"] },
  { value: "less",          label: "nh? hon",      types: ["NUMBER","LINEAR_SCALE","RATING"] },
  { value: "answered",      label: "dã tr? l?i",    types: ["TEXT","PARAGRAPH","EMAIL","NUMBER","DATE","TIME","SINGLE_CHOICE","MULTIPLE_CHOICE","DROPDOWN","RATING","LINEAR_SCALE"] },
    ...
    if (isChoice) {
      ...
      return (
        <select value={condValue}
          ...
          <option value="">— Ch?n dáp án —</option>
          {opts.map((o,i) => (
    ...
    if (["NUMBER","LINEAR_SCALE","RATING"].includes(srcFEType)) {
      return (
        <input type="number" value={condValue}
          onChange={e => onChange({ source_question_id: sourceId, operator, value: e.target.value })}
          placeholder="Giá tr?..." style={{...inp(false), padding:"5px 8px", fontSize:12, flex:1}}/>
      );
    }
    return (
  ...
  return (
    <div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:4}}>
        <span style={{...lbl, marginBottom:0}}>Ði?u ki?n hi?n th? (Skip logic)</span>
        <button onClick={() => setOpen(v => !v)} style={{
          ...
          {hasCondition ? "? Có di?u ki?n" : "+ Thêm di?u ki?n"}
        </button>
      </div>
      ...
      {open && (
        <div style={{
          ...
          <p style={{fontSize:11,color:C.textSub,margin:0,lineHeight:1.4}}>
            Câu h?i này ch? hi?n th? khi di?u ki?n bên du?i du?c th?a mãn.
          </p>

          <div>
            <span style={{fontSize:11,color:C.textSub,display:"block",marginBottom:4}}>Câu h?i ngu?n</span>
          ...
          {sourceId && (
            <div>
              <span style={{fontSize:11,color:C.textSub,display:"block",marginBottom:4}}>Ði?u ki?n</span>
          ...
          {sourceId && operator && !["answered","not_answered"].includes(operator) && (
            <div>
              <span style={{fontSize:11,color:C.textSub,display:"block",marginBottom:4}}>Giá tr?</span>
              {getValueInput()}
            </div>
          )}
...
function QuestionCard({ q, index, isActive, onActivate, onSave, onCancel, onDelete, onDuplicate, deletingId, sections, questions, surveyId }) {
  ...
  const handleMediaUpload = async (e) => {
    ...
    if (file.size > 5 * 1024 * 1024) { alert("File quá l?n. T?i da 5MB."); return; }
      ...
      >
        ...
        <p
          ...
          dangerouslySetInnerHTML={{
            __html: q.content
              ? q.content
              : "<em style=\"color:" + C.textDim + ";font-weight:400\">Câu h?i chua có tiêu d?</em>"
          }}
        />
...
{sections?.length > 0 && (
  <select
    ...
    style={{
      ...
      flexShrink: 0}}
  >
    <option value="">Chua phân trang</option>
    {sections.map(s => (
  ...
  return (
    <div style={{
      ...
      <div style={{display:"flex",alignItems:"flex-start",gap:12,padding:"16px 20px 0"}}>
        <div style={{flex:1}}>
          <span style={{...lbl,marginBottom:6}}>N?i dung câu h?i</span>
          <RichTextEditor
            value={contentHtml}
            onChange={setContentHtml}
            placeholder="Câu h?i không có tiêu d?"
            minHeight={56}
          />
        </div>
      ...
      <div style={{padding:"8px 20px 0",paddingLeft:36,display:"flex",flexDirection:"column",gap:8}}>
        <div>
          <span style={{...lbl, marginBottom:4}}>Mô t? câu h?i (tùy ch?n)</span>
          <input
            type="text"
            value={description}
            onChange={e => setDescription(e.target.value)}
            placeholder="Thêm g?i ý ho?c mô t? cho câu h?i..."
            style={{...inp(false), padding:"7px 10px", fontSize:12}}
          />
        </div>
        {["TEXT","PARAGRAPH","EMAIL","DATE","NUMBER","TIME"].includes(type) && (
          <div>
            <span style={{...lbl, marginBottom:4}}>Placeholder (tùy ch?n)</span>
            <input
              type="text"
              value={placeholder}
              onChange={e => setPlaceholder(e.target.value)}
              placeholder="Van b?n g?i ý trong ô nh?p li?u..."
              style={{...inp(false), padding:"7px 10px", fontSize:12}}
            />
          </div>
        )}
      ...
      <div style={{padding:"8px 20px 0",paddingLeft:36}}>
        <div style={{display:"flex",flexDirection:"column",gap:4}}>
          <span style={{...lbl, marginBottom:4}}>Hình ?nh / Video (URL tùy ch?n)</span>
      ...
      <div style={{padding:"8px 20px 0",paddingLeft:36}}>
        ...
        >
          <option value="">— Không thu?c trang nào —</option>
          {sections.map(s => (
            <option key={s.id} value={s.id}>{s.title || "Không có tiêu d?"}</option>
          ))}
        </select>
      </div>
      ...
      <div style={{
        ...
        <button onClick={()=>onDuplicate(q)} title="Nhân dôi"
...
function Sidebar({ onAddQuestion }) {
  const items = [
    { icon:<Plus size={18}/>,    title:"Thêm câu h?i",   action:onAddQuestion },
    { icon:<FileText size={18}/>,title:"Import câu h?i", action:()=>{} },
    { icon:<Type size={18}/>,    title:"Thêm tiêu d?",   action:()=>{} },
    { icon:<Image size={18}/>,   title:"Thêm hình ?nh",  action:()=>{} },
    { icon:<Video size={18}/>,   title:"Thêm video",     action:()=>{} },
    { icon:<Minus size={18}/>,   title:"Thêm ph?n m?i",  action:()=>{} },
  ];
  return (
    <div style={{
      ...
      {!editing ? (
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 600, color: "#111827", margin: "8px 0 10px", lineHeight: 1.25 }}>
              {title?.trim() ? title : "Chua d?t tiêu d?"}
            </h1>
            <p style={{ margin: 0, fontSize: 14, color: C.textMuted, lineHeight: 1.55, whiteSpace: "pre-wrap" }}>
              {description?.trim() ? description : "Chua có mô t? cho kh?o sát này."}
            </p>
          </div>
          <button
            type="button"
        ...
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            <div>
              <span style={{ ...lbl, marginBottom: 6, display: "block" }}>Tiêu d?</span>
              <input
                type="text"
                value={draftTitle}
                onChange={(e) => { setDraftTitle(e.target.value); setLocalErr(""); }}
                placeholder="Tên kh?o sát"
                style={inp(false)}
              />
            </div>
            <div>
              <span style={{ ...lbl, marginBottom: 6, display: "block" }}>Mô t?</span>
              <textarea
                value={draftDesc}
                onChange={(e) => { setDraftDesc(e.target.value); setLocalErr(""); }}
                placeholder="Mô t? ng?n (tu? ch?n)"
                rows={3}
                style={inp(true)}
              />
            </div>
          ...
          <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 16 }}>
            ...
            <button
              ...
              onClick={async () => {
                ...
                if (!t) { setLocalErr("Tiêu d? không du?c d? tr?ng."); return; }
                try {
...
const fetchSections = useCallback(async (sid) => {
  ...
  const handleAdd = async (e) => {
    ...
    const plainContent = getPlainText(contentHtml).trim();
    if (!plainContent) {
      setFormError("N?i dung câu h?i không du?c d? tr?ng.");
      return;
    }
    ...
    if (isChoice) {
      ...
      if (valid.length < 2) {
        setFormError("C?n ít nh?t 2 l?a ch?n h?p l? (label và value không du?c r?ng).");
        return;
      }
    }
    ...
    if (qImage?.file) {
      try {
        ...
        toast.error("Upload ?nh th?t b?i. Câu h?i s? du?c t?o không có ?nh.");
      }
    }
        ...
        <div style={{flex:1,minWidth:0,display:"flex",flexDirection:"column",gap:14}}>
          ...
          <div style={{
            ...
            <div style={{display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
              ...
              <span style={{fontSize:13,color:C.textSub,fontWeight:500}}>{questions.length} câu h?i</span>
              <button
                type="button"
          ...
          {showForm && (
            <div style={{
              ...
              padding:20}}>
              <h2 style={{fontSize:16,fontWeight:600,color:"#111827",margin:"0 0 16px"}}>Câu h?i m?i</h2>
              <form onSubmit={handleAdd}>
                <div style={{display:"flex",flexDirection:"column",gap:14}}>

                  <div style={{display:"flex",gap:12,alignItems:"flex-start"}}>
                    <div style={{flex:1}}>
                      <span style={lbl}>N?i dung câu h?i *</span>
                      <RichTextEditor
                        value={contentHtml}
                        onChange={(html) => { setContentHtml(html); setFormError(""); }}
                        placeholder="Nh?p n?i dung câu h?i..."
                        minHeight={72}
                        hasError={!!formError && !getPlainText(contentHtml).trim()}
                      />
                    </div>
                    <div style={{minWidth:200}}>
                      <span style={lbl}>Lo?i câu h?i</span>
                  ...
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    <div>
                      <span style={lbl}>Mô t? câu h?i (tùy ch?n)</span>
                      <input type="text" value={description} onChange={e => setDescription(e.target.value)}
                        placeholder="Thêm g?i ý ho?c mô t?..."
                        style={{...inp(false), padding:"7px 10px", fontSize:12}}/>
                    </div>
                    {["TEXT","PARAGRAPH","EMAIL","DATE","NUMBER","TIME"].includes(type) && (
                      <div>
                        <span style={lbl}>Placeholder (tùy ch?n)</span>
                    ...
                    <div>
                      <span style={lbl}>Hình ?nh / Video (tùy ch?n)</span>
                      <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap"}}>
                        <input
                          ...
                          onChange={async (e) => {
                            const file = e.target.files?.[0];
                            if (!file) return;
                            if (file.size > 5 * 1024 * 1024) { toast.error("File quá l?n. T?i da 5MB."); return; }
                  ...
                  <div style={{display:"flex",justifyContent:"flex-end",gap:10}}>
                    ...
                    <button type="submit" disabled={formLoading} style={{
                      ...
                      {formLoading ? "Ðang thêm..." : "Thêm câu h?i"}
                    </button>
                  </div>
          ...
          {loading && questions.length === 0 ? (
            ...
            <div style={{
              ...
              <p style={{fontSize:15,margin:0,color:C.text,fontWeight:600}}>Chua có câu h?i nào</p>
              <p style={{fontSize:13,margin:0,color:C.textSub}}>Hãy t?o câu h?i d?u tiên ho?c nh? AI g?i ý</p>
              ...
              {activeSectionId && (() => {
                ...
                return sec ? (
                  <div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 16px",background:C.primaryBg,borderRadius:8,border:"1px solid " + C.primaryLight}}>
                    ...
                    <span style={{fontSize:12,color:C.textSub,marginLeft:"auto"}}>{displayedQuestions.length} câu h?i</span>
                  </div>
                ) : null;
              })()}

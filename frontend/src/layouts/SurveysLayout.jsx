        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            ...
            <div style={{ display:"flex", justifyContent:"flex-end", gap:8, marginTop:4 }}>
              <button type="button" onClick={onClose} style={{ padding:"9px 16px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", color:C.textSub, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:C.font }}>Đóng</button>
    ...
    <Modal open={open} onClose={onClose} title={isPublished ? "Ẩn khảo sát" : "Publish khảo sát"} width={400}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", justifyContent:"flex-end", gap:8 }}>
          <button onClick={onClose} style={{ padding:"9px 16px", borderRadius:9, border:`1px solid ${C.border}`, background:"transparent", color:C.textSub, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:C.font }}>Huỷ</button>
...
function MySurveyCard({ survey, index, onDelete, onUpdate, onShare, onInvite, onPublish, onCloseSurvey }) {
  ...
  const menuItems = [
    ...
    { icon:<Users size={13}/>,   label:"Mời người dùng",   action:() => { setInviteOpen(true); setMenuOpen(false); } },
      ...
      >
        ...
        <div style={{ height:110, background:isClosed ? "linear-gradient(135deg,#f1f5f9,#e2e8f0)" : thumb, position:"relative", display:"flex", alignItems:"center", justifyContent:"center" }}>
          ...
          <div ref={menuRef} style={{ position:"absolute", top:8, right:8 }} onClick={e => e.stopPropagation()}>
            ...
            {menuOpen && (
              <div style={{ position:"absolute", top:34, right:0, width:180, background:"#fff", border:`1px solid ${C.border}`, borderRadius:12, overflow:"hidden", boxShadow:"0 12px 40px rgba(0,0,0,.12)", zIndex:20, animation:"slideUp .12s ease" }}>
          ...
          {editing ? (
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              ...
              <div style={{ display:"flex", justifyContent:"flex-end", gap:6 }}>
                <button onClick={() => setEditing(false)} style={{ padding:"7px 12px", borderRadius:8, border:`1px solid ${C.border}`, background:"transparent", cursor:"pointer", fontSize:12, fontWeight:600, color:C.textSub, fontFamily:C.font }}>Huỷ</button>
...
function PublicSurveyCard({ survey, done, onStart, onViewSubmission }) {
  ...
  return (
    <div
      ...
      style={{
        background:"#fff",
        border:`1px solid ${done ? "rgba(16,185,129,0.2)" : C.border}`,
        borderRadius:14,
        padding:18,
      ...
      <form onSubmit={(e) => { e.preventDefault(); onSubmit(formData); }}>
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <div style={{ display:"flex", justifyContent:"flex-end", gap:7, marginTop:2 }}>
            <button type="button" onClick={onCancel} style={{ padding:"8px 14px", borderRadius:8, border:`1px solid ${C.border}`, background:"transparent", color:C.textSub, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:C.font }}>Huỷ</button>
...
/* ════════════════════════════════════════════════════════════════
   MAIN PAGE
════════════════════════════════════════════════════════════════ */
const MY_SURVEYS_PREVIEW = 4; // số card hiển thị mặc định
...
export default function SurveysLayout() {
  ...
  const handleSubmitCreate = async (formData) => {
    try {
      ...
      setMyExpanded(true); // auto-expand để thấy survey mới
      fetchMySurveys(1, 20);
  ...
  // My surveys: hiển thị preview hoặc tất cả
    ...
    <main style={{ minHeight:"100vh", background:C.bg, fontFamily:C.font }}>
      ...
      <div style={{
        ...
        <div style={{ flex:1, maxWidth:400, height:36, borderRadius:999, background:C.surfaceHigh, border:`1px solid ${C.border}`, display:"flex", alignItems:"center", gap:8, padding:"0 14px", transition:"border-color .15s" }}
          onFocus={e => e.currentTarget.style.borderColor="rgba(67,97,238,0.3)"}
                ...
                <>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:14 }}>
                    {visibleMySurveys.map((survey, index) => (
                      <MySurveyCard key={survey.id} survey={survey} index={index}
                  ...
                  {hasMoreMySurveys && !showCreateForm && (
                    <div style={{ display:"flex", justifyContent:"center", marginTop:16 }}>
                      <button
                        ...
                        style={{ display:"flex", alignItems:"center", gap:7, padding:"8px 20px", borderRadius:999, border:`1px solid ${C.border}`, background:"#fff", color:C.textSub, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:C.font, transition:"all .15s", boxShadow:"0 1px 4px rgba(0,0,0,.06)" }}
                      ...
                      >
                        {myExpanded
                          ...
                          : <><ChevronDown size={13}/> Xem thêm {myFiltered.length - MY_SURVEYS_PREVIEW} survey</>
                        }
                      </button>
                    </div>
                  )}
